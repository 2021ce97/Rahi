...
if (!code.includes('const [selectedBadges')) {
...
if (!code.includes('selectedBadges.includes(b)')) {
...
if (code.includes('rating: rating') && !code.includes('selectedBadges')) {
    code = code.replace(/rating: rating/, `rating: rating, badges: selectedBadges`);
}

if (code.match(/You have no active or requesting rides\./) && !code.includes('*123#')) {
